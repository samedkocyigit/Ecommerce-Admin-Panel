<template>
  <div class="branch-parent">
    ...
    <UserRoleModal
      v-if="roleModalStatus"
      :user="user"
      @close="modalToggle('role', null)"
    ...
    <DeleteModal
      ...
      :user="user"
      @close="modalToggle('delete', null)"
      @toastAlert="toastAlert"
    ></DeleteModal>

    <!-- Detail Modal Component -->
    <DetailModal
      v-if="detailModalStatus"
      :user="user"
      @close="modalToggle('detail', null)"
    ></DetailModal>
    ...
    <div class="main" :class="{ toggleWidth: getToggleStatus }">
      ...
      <div class="table-box">
        ...
        <div class="branch-nav">
          <div
            class="nav-item"
            :class="{ active: customerTableStatus }"
        ...
        <!-- Customer Table -->
        <CustomerTable
          v-if="customerTableStatus"
          @showModal="modalToggle"
        ></CustomerTable>

        <!-- Admin Table -->
        <AdminTable
          v-if="adminListStatus"
          @showModal="modalToggle"
        ></AdminTable>
      </div>
      <!-- Table Box End -->
...
export default {
  ...
  methods: {
    ...
    modalToggle(status, id) {
      if (id !== null) {
        let userData = [...this.getCustomers, ...this.getAdmins].find(
          (user) => user._id === id,
        );
        if (userData) {
          this.user = userData; // Kullanıcı verisini güncelliyoruz
        }
      } else {
        this.user = null;
      }
