...
export default {
  ...
  getters: {
    ...
    paginatedCategories: (state) =>
      state.categories.slice(
        state.categoryPaginationPoints.start,
        state.categoryPaginationPoints.end,
      ),
  ...
  mutations: {
    ...
    setTakenCategories: (state, categories) =>
      (state.simpleCategories = categories),
  ...
  actions: {
    ...
    deleteCategory: async ({ commit }, removeId) => {
      await axios.delete(`http://127.0.0.1:3000/api/categories/${removeId}`);
    ...
    updateCategory: async ({ commit }, newCategory) => {
      let { data } = await axios.post(
        `http://127.0.0.1:3000/api/categories/${newCategory.id}`,
        newCategory,
      );
