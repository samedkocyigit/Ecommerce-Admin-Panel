...
export default {
  ...
  getters: {
    ...
    paginatedAdmins: (state) =>
      state.admins.slice(
        state.adminPaginationPoints.start,
        state.adminPaginationPoints.end,
      ),
    ...
    getAdminCurrentPage: (state) => {
      return (
        state.adminPaginationPoints.end /
        (state.adminPaginationPoints.end - state.adminPaginationPoints.start)
      );
    },
  ...
  actions: {
    ...
    allAdmins: async ({ commit }) => {
      let { data } = await axios.get('http://127.0.0.1:3000/api/users/admin');
      console.log('admins', data.data.data);
      commit('setAdmins', data.data.data);
    },

    /* Get Paginated Customer Data */
    allCustomers: async ({ commit }, url) => {
      let URL = url ? url : 'http://127.0.0.1:3000/api/users';
    ...
    deleteUser: async ({ commit }, removeId) => {
      await axios.delete(`http://127.0.0.1:3000/api/users/${removeId}`);
    ...
    updateUser: async ({ commit }, updateUserData) => {
      let { data } = await axios.post(
        `http://127.0.0.1:3000/api/users/${updateUserData.id}`,
        updateUserData,
      );
    ...
    changeUserRole: async ({ commit }, newRoleData) => {
      let { data } = await axios.post(
        'https://nooneuse.shop/api/admin/user/changeRole',
        newRoleData,
      );
    ...
    searchUser: async ({ commit }, searchKey) => {
      const { data } = await axios.get(
        `https://nooneuse.shop/api/admin/user/getAllCustomers/${searchKey}`,
      );
      data.data = await data.data.filter(
    ...
    myProfile: async ({ commit }) => {
      let { data } = await axios.get(
        'http://127.0.0.1:3000/api/users/' + localStorage.getItem('ID'),
      );
      console.log('myprofile', data.data.data);
      commit('setMyProfile', data.data.data);
    },
  },
};
