  ...
  <transition name="side" :appear="this.$route.name !== 'auth'">
    <div class="sideBar" :class="{ toggleWidth: getToggleStatus }">
      ...
      <ul>
        ...
        <li>
          <router-link to="/slider" active-class="active" class="side-btn">
            <div class="link-container">
              <div class="icon"><i class="fa-solid fa fa-sliders"></i></div>
              <div class="text-title">Slider</div>
            </div>
          </router-link>
        </li>
        <li>
...
.sideBar {
  ...
  top: 0;
  left: 0;
  background-color: rgba(15, 0, 146, 0.582);
  overflow: hidden;
  z-index: 1000;
  transition: 0.5s;
}
