    ...
    <table v-if="getProducts && getProducts.length !== 0">
      ...
      <tbody>
        <tr v-for="(product, index) in getProducts" :key="index">
          <td class="photo">
            <img :src="`/images/products/${product.photos_detail[0]}`" alt="" />
          </td>
          <td class="name">{{ product.name }}</td>
          <td class="category">{{ product.categoryId }}</td>
          <td class="price p">{{ product.price }}</td>
          <td class="view vc">{{ product.ratingsAverage }}</td>
          <td class="count re">{{ product.stock_code }}</td>
          <td class="updated">{{ product.createdAt }}</td>
          <td class="control btns">
            <button @click="showProductUpdatePage(product.id)" title="edit">
              <i class="fa-regular fa-pen-to-square"></i>
            </button>
            <button @click="$emit('deleteProduct', product.id)" title="delete">
              <i class="fa-regular fa-trash-can"></i>
            </button>
            <button @click="showProductDetailPage(product.id)" title="info">
...
export default {
  ...
  methods: {
    ...
    async showProductUpdatePage(id) {
      ...
      this.$emit('productUpdate', data.data.data);
    },
  },
};
</script>
